...
const items = [
  {
    ...
    mark: t['dark-blue'], wm: t['dark-blue'],
  },
  {
        ...
        {items.map((item, i) => (
          <div key={item.heading} style={{
            ...
            <div style={{ background: item.bg, height: 160, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", borderBottom: "1px solid #E5E5E5" }}>
              {item.isRetype
                ? <span style={{ fontFamily: "Georgia, serif", fontSize: 24, fontWeight: 700, color: "var(--black)", letterSpacing: "-0.01em" }}>{brand.meta.client}</span>
                : <MainWordmarkSvg
            ...
            <div style={{ padding: "20px 24px" }}>
              ...
              <div style={{ fontFamily: "Saans, sans-serif", fontSize: 13, color: "var(--black)", lineHeight: 1.5 }}>{item.body}</div>
            </div>
          </div>
        ))}
      </div>
